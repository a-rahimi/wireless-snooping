...
def plot_audio_samples(
    ...
    for i, (row_axes, sample) in enumerate(zip(axes, samples)):
        ...
        phase_arr = sample.phase_velocity()
...
def decode_subpacket(
    ...
    preamble_target: int | None= 0x552dd4,
) -> np.ndarray:
    ...
    if preamble_target is not None:
        data_bits = decode.find_after(data_bits, preamble_target)
    ...
    pre = "".join(str(int(b)) for b in binarized.bits[max(0, transition - 8):transition])
    post = "".join(str(int(b)) for b in binarized.bits[transition:transition + 8])
    print(f"  preamble -> data: ...({transition} bits) {pre} | {post} ({len(data_bits)} bits)...")
...
def find_run_of_high_amplitude(sample: packets.DemodPacket, trim_sample: int = 10, min_samples: int = 6000):
    ...
    return zip(trimmed_starts[mask], trimmed_ends[mask])

def decode_packet(sample: packets.DemodPacket, show_plot: bool = True):
    for srange in find_run_of_high_amplitude(sample):
        data_uint8 = decode_subpacket(sample, srange, show_plot)
        print(decode.uint8_to_hex(data_uint8))


decode_packet(audio[3])


# %%
for ipacket, sample in enumerate(audio):
    for srange in find_run_of_high_amplitude(sample):
        data_uint8 = decode_subpacket(sample, srange, preamble_target=0x552dd4, show_plot=False)
        if data_uint8 is None:
            continue
        hex = decode.uint8_to_hex(data_uint8)

        print("\n=====", ipacket,  sample.timestamp - audio[0].timestamp, "sec")
        print(hex)
        print(0.1 * (((data_uint8[3] & 0x0f)  *256.) + data_uint8[4]))
